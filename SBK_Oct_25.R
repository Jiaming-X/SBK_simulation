...
generate1=function(n, d)
{
  ...
  m1 =  2/(x^2+3)#sin(2*pi*x)# 1/(x^2+2)
  m2 = (-1/16)*cos(2*pi*x)#sin(2*pi*x)# (-1/16)*cos(2*pi*x)
  m = m1
  m[,2] = m2[,2]
  e = rnorm(n)
  y = sapply(1:n, function(j) sum(m[j,]) + e[j])
...
plot(re, main = "n = 50", xlim = globaly) # plots the results
...
###No Sorting for the x, y, mr, yo, yp\ the order can make the problems
